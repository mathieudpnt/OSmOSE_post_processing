...
df_FPOD_start_dt = [pytz.timezone(tz_data).localize(pd.to_datetime(x, format="%d/%m/%Y %H:%M")) for x in df_FPOD['Date heure']]
# Compute the absolute end date time of detection
df_FPOD_end_dt = [x + timedelta(seconds = det_bin_size) for x in df_FPOD_start_dt]
# Build the dataframe
data = {'dataset' : [dataset_name]*nb_det, 'filename' : ['']*nb_det, 'start_time' : [0]*nb_det, 'end_time' : [det_bin_size]*nb_det, 'start_frequency' :  [0]*nb_det, 'end_frequency' :  [0]*nb_det,'annotation' : [species]*nb_det, 'annotator' : ['FPOD']*nb_det, 'start_datetime' : df_FPOD_start_dt,  'end_datetime' : df_FPOD_end_dt}
