...
def suntime_hour(begin_deploy, end_deploy, lat, lon):
    ...
    if np.isnan(lat): lat = 48  # arbitrary location
    if np.isnan(lon): lon = -5  # arbitrary location
...
def stats_diel_pattern(deployment: pd.Series, detector: str):
    ...
    assert 'datetime deployment' in deployment.index and isinstance(deployment['datetime deployment'], pd.Timestamp), "Error datetime deployment"
    assert 'datetime recovery' in deployment.index and isinstance(deployment['datetime recovery'], pd.Timestamp), "Error datetime recovery"
    assert 'datetime recovery' in deployment.index and isinstance(deployment['datetime recovery'], pd.Timestamp), "Error datetime recovery"
    assert f'df {detector}' in deployment.index and isinstance(deployment[f'df {detector}'], pd.DataFrame), f"Error f'df {detector}'"
    assert 'latitude' in deployment.index and pd.api.types.is_numeric_dtype(deployment['latitude']), "Error latitude"
    assert 'longitude' in deployment.index and pd.api.types.is_numeric_dtype(deployment['longitude']), "Error longitude"

    df_detections = deployment[f'df {detector}']
    timeZ = pytz.FixedOffset(deployment['datetime deployment'].utcoffset().total_seconds() // 60)

    # Compute sunrise and sunset decimal hour at the dataset location
    [dt_dusk, dt_dawn, dt_day, dt_night] = suntime_hour(begin_deploy=deployment['datetime deployment'],
    ...
    for idx, day in enumerate(list_day):
        ...
        if pd.notna(night1_duration) or pd.notna(night2_duration):
            night_combine = (night1_duration if pd.notna(night1_duration) else pd.Timedelta(0, 'seconds')) + (night2_duration if pd.notna(night2_duration) else pd.Timedelta(0, 'seconds'))
    ...
    dawn_duration_dec = [(dawn_d.total_seconds() / 3600) if pd.notna(dawn_d) else float('NaN') for dawn_d in dawn_duration]
    day_duration_dec = [(day_d.total_seconds() / 3600) if pd.notna(day_d) else float('NaN') for day_d in day_duration]
    dusk_duration_dec = [(dusk_d.total_seconds() / 3600) if pd.notna(dusk_d) else float('NaN') for dusk_d in dusk_duration]
    night_duration_dec = [(night_d.total_seconds() / 3600) if pd.notna(night_d) else float('NaN') for night_d in night_duration]

    night_duration_dec2 = []
    for i in range(len(night_duration_dec)):
        night_duration_dec2.append(np.nansum([night_duration_dec[i], dusk_duration_dec[i], dawn_duration_dec[i]]))
