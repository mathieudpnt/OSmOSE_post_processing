...
from utilities.def_func import reshape_timebin2, t_rounder
...
def plot_single(deployment: Deployment, file: str, timebin: int = 60, begin_date: pd.Timestamp = None, end_date: pd.Timestamp = None, fmin_filter: int = None, fmax_filter: int = None) -> None:
    ...
    if fmin_filter is not None:
        ...
        assert (len(df) > 0), f"No detection found in {file[0]} DataFrame after fmin filtering at {fmin_filter} Hz, upload aborted"
    ...
    df_reshaped = reshape_timebin2(df=df, timebin_new=timebin, timestamp=timestamp2, reshape_method='timestamp')

    # list of the labels corresponding to the selected user
    if isinstance(annotation, str):
        label_ref = annotation
    else:
        label_ref = easygui.buttonbox(f'Select a label for {annot_ref}', 'Single plot', annotation)

    res_min = easygui.integerbox('Enter the bin size (min) ', 'Time resolution', default=10, lowerbound=1, upperbound=86400)
    delta, start_vec, end_vec = dt.timedelta(seconds=60 * res_min), t_rounder(begin_date, res=600), t_rounder(end_date + dt.timedelta(seconds=timebin), res=600)
    time_vector = [start_vec + i * delta for i in range(int((end_vec - start_vec) / delta) + 1)]
    n_annot_max = (res_min * 60) / timebin  # max nb of annoted time_bin max per res_min slice
    df_1annot_1label = df_reshaped[(df_reshaped['annotator'] == annot_ref) & (df_reshaped['annotation'] == label_ref)]
...
def get_agreement(deployment: Deployment, file: str, timebin: int = 60, begin_date: pd.Timestamp = None, end_date: pd.Timestamp = None, fmin_filter: int = None, fmax_filter: int = None) -> None:
    ...
    assert isinstance(annotator, List) and len(annotator) == 2, 'Agreement plot cancelled, not enough annotators to make a comparison'
    if len(annotator) > 2:
        annot_ref1 = easygui.buttonbox('Select annotator 1', 'Plot label', annotator)
        annot_ref2 = easygui.buttonbox('Select an annotator', 'Plot label', [elem for elem in annotator if elem != annot_ref1])
    ...
    if not isinstance(annotation, str):
        label_ref = easygui.buttonbox('Select the label to plot', 'Single plot', annotation)
    else:
        label_ref = annotation
    ...
    df1_1annot_1label = df_reshaped[(df_reshaped['annotator'] == annot_ref1) & (df_reshaped['annotation'] == label_ref)]
    df2_1annot_1label = df_reshaped[(df_reshaped['annotator'] == annot_ref2) & (df_reshaped['annotation'] == label_ref)]
...
def get_perf(deployment: Deployment, file: List[str], timebin: int = 60, begin_date: pd.Timestamp = None, end_date: pd.Timestamp = None, fmin_filter: List[int] = None, fmax_filter: List[int] = None) -> None:
    ...
    if fmax_filter is not None:
        assert isinstance(fmax_filter, List), "A list of 2 integers must be passed for fmin_filter"
        assert len(fmax_filter) == 2, "A list of 2 integers must be passed for fmax_filter"
    if all(isinstance(f, List) for f in [fmin_filter, fmax_filter]) and all(len(f) == 2 for f in [fmin_filter, fmax_filter]):
        for i in range(2):
    ...
    if fmin_filter is not None:
        ...
        assert (len(df1) > 0), f"No detection found in {file[0]} DataFrame after fmin filtering at {fmin_filter} Hz, upload aborted"
        assert (len(df2) > 0), f"No detection found in {file[1]} DataFrame after fmin filtering at {fmin_filter} Hz, upload aborted"
    ...
    t2 = t_rounder(min(timestamp2[-1], end_date), res=timebin) + pd.Timedelta(seconds=timebin)
    time_vector = [ts.timestamp() for ts in pd.date_range(start=t, end=t2, freq=str(timebin) + 's')]

    tz = pytz.FixedOffset(begin_date.utcoffset().total_seconds() // 60)

    # annotator
    if len(annotator1) > 1 and not isinstance(annotator1, str):
        selected_annotator1 = easygui.buttonbox('Select annotator', 'Performances', annotator1)
    elif isinstance(annotator1, str):
        selected_annotator1 = annotator1

    if len(annotator2) > 1 and not isinstance(annotator2, str):
        selected_annotator2 = easygui.buttonbox('Select annotator', 'Performances', annotator2)
    elif isinstance(annotator2, str):
        selected_annotator2 = annotator2

    # label
    if len(annotation1) > 1 and not isinstance(annotation1, str):
        selected_label1 = easygui.buttonbox('Select annotation', 'Performances', annotation1)
    elif isinstance(annotation1, str):
        selected_label1 = annotation1

    if len(annotation2) > 1 and not isinstance(annotation2, str):
        selected_label2 = easygui.buttonbox('Select annotation', 'Performances', annotation2)
    elif isinstance(annotation2, str):
        selected_label2 = annotation2

    # df1 - REFERENCE
    df1_reshaped = reshape_timebin2(df=df1,
                                    timebin_new=timebin,
                                    reshape_method='timebin',
                                    timestamp=timestamp)

    selected_annotations1 = df1_reshaped[(df1_reshaped['annotator'] == selected_annotator1) & (df1_reshaped['annotation'] == selected_label1) & (df1_reshaped['start_datetime'] >= begin_date) & (df1_reshaped['end_datetime'] <= end_date)]
    ...
    for i in range(len(times1_beg)):
        for j in range(k, len(time_vector) - 1):
            if (times1_beg[i] >= time_vector[j] and times1_beg[i] < time_vector[j + 1]) or (times1_end[i] > time_vector[j] and times1_end[i] <= time_vector[j + 1]):
                ranks.append(j)
                k = j
                break
            else:
    ...
    # df2
    df2_reshaped = reshape_timebin2(df=df2,
                                    timebin_new=timebin,
                                    reshape_method='timebin',
                                    timestamp=timestamp)

    selected_annotations2 = df2_reshaped[(df2_reshaped['annotator'] == selected_annotator2) & (df2_reshaped['annotation'] == selected_label2) & (df2_reshaped['start_datetime'] >= begin_date) & (df2_reshaped['end_datetime'] <= end_date)]
    ...
    for i in range(len(times2_beg)):
        for j in range(k, len(time_vector) - 1):
            if (times2_beg[i] >= time_vector[j] and times2_beg[i] < time_vector[j + 1]) or (times2_end[i] > time_vector[j] and times2_end[i] <= time_vector[j + 1]):
                ranks.append(j)
                k = j
                break
            else:
    ...
    # plot
    res_min = easygui.integerbox('Enter the bin size (min) ', 'Time resolution', default=10, lowerbound=1, upperbound=86400)
    delta, start_vec, end_vec = pd.Timedelta(seconds=60 * res_min), t_rounder(begin_date, res=600), t_rounder(end_date + pd.Timedelta(seconds=timebin), res=600)
    bin_vector = [start_vec + i * delta for i in range(int((end_vec - start_vec) / delta) + 1)]
    n_annot_max = (res_min * 60) / timebin  # max nb of annoted time_bin max per res_min slice

    fig, ax = plt.subplots(1, 1, dpi=200, figsize=(10, 4))

    ax.hist([selected_annotations1['start_datetime'], selected_annotations2['start_datetime']], bins=bin_vector, label=[selected_label1, selected_label2], edgecolor='black')
    ...
    ax.tick_params(axis='y')
    fig.suptitle(f'[{selected_annotator1}/{selected_label1}] VS [{selected_annotator2}/{selected_label2}]', y=1.02)
