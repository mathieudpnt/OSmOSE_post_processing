...
def get_data(dt1, dt2, file_list):
    ...
    selected_files = file_list[np.logical_or(np.logical_or(
...
dir_file = os.path.dirname('L:/acoustock3/Bioacoustique/DATASETS/MIRACETI_ACOUSTIQUE/2018/')
...
for file in lst_mp3:
    ...
    new_fn = os.path.join(dir_reformatted, os.path.basename(file).split('.')[0] + '_formatted.wav')
...
df_file = pd.DataFrame(index=range(len(lst_file_formatted)), columns=['filename', 'dtstart', 'dtend', 'nbech', 'duration', 'sample_rate'])
...
for i, file in enumerate(lst_file_formatted):
    ...
    while dt_begin <= dt_end:
        ...
        data_norm = np.where(data_out > 0, data_out / data_out.max(), np.where(data_out < 0, -data_out / data_out.min(), data_out))
