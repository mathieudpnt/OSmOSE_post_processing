...
class Deployment:
    ...
    Attributes:
        path_json (Union[Path, str]): Path to the JSON file containing metadata.
        campaign (Union[str, int]): Campaign identifier.
        deployment (Union[str, int]): Deployment identifier.
        recorder (Union[str, int]): Recorder identifier.
        path_metadata (Union[Path, str]): Path to the file containing file metadata.
        path_origin_metadata (Union[Path, str]): Path to the file containing origin metadata.
        path_origin_timestamp (Union[Path, str]): Path to the file containing origin timestamps.
        path_segment_metadata (Union[Path, str]): Path to the file containing segment metadata.
        path_segment_timestamp (Union[Path, str]): Path to the file containing segment timestamps.
        path_pamguard (Union[Path, str]): Path to the PAMGuard annotations file.
        path_thalassa (Union[Path, str]): Path to the Thalassa annotations file.
        path_aplose (Union[Path, str]): Path to the APLOSE annotations file.
    """

    def __init__(
            self,
            path_json: Union[Path, str] = None,

            campaign: Union[str, int] = None,
            deployment: Union[str, int] = None,
            recorder: Union[str, int] = None,
            path_metadata: Union[Path, str] = None,
            path_origin_metadata: Union[Path, str] = None,
            path_origin_timestamp: Union[Path, str] = None,
            path_segment_metadata: Union[Path, str] = None,
            path_segment_timestamp: Union[Path, str] = None,

            path_pamguard: Union[Path, str] = None,
            path_thalassa: Union[Path, str] = None,
            path_aplose: Union[Path, str] = None,

    ) -> None:

        argument = [campaign, deployment, recorder, path_metadata, path_origin_metadata, path_origin_timestamp, path_segment_metadata, path_segment_timestamp]
        assert (all(arg is not None for arg in argument) or path_json is not None), "Argument(s) missing"
        assert all(isinstance(arg, Union[str, int]) or arg is None for arg in [campaign, deployment, recorder]), 'Input arguments error'
    ...
    def load_json(self, path_json):
        ...
        try:
            ...
            self.load_metadata(self.campaign, self.deployment, self.recorder, self.path_metadata, self.path_origin_metadata, self.path_origin_timestamp, self.path_segment_metadata, self.path_segment_timestamp, self.path_pamguard, self.path_thalassa, self.path_aplose)
    ...
    def load_metadata(self, campaign, deployment, recorder, path_metadata, path_origin_metadata, path_origin_timestamp, path_segment_metadata, path_segment_timestamp, path_pamguard, path_thalassa, path_aplose):
        ...
        try:
            ...
            attributes.append(['metadata', 'origin_sr', 'audio_file_count', 'start_date', 'end_date', 'gps_coordinates', 'depth'])

            # origin  metadata
            self.origin_metadata = self.load_csv(path_origin_metadata, parse_dates=['timestamp'])
            self.origin_duration = self.origin_metadata['duration'].to_list()
            attributes.append(['origin_metadata', 'origin_duration'])

            # originin timestamp
            self.origin_timestamp_df = self.load_csv(path_origin_timestamp, parse_dates=['timestamp'])
            ...
            # segment_timestamp
            self.segment_timestamp_df = self.load_csv(path_segment_timestamp, parse_dates=['timestamp'])
            ...
            flat_attributes = [item for sublist in attributes for item in (sublist if isinstance(sublist, list) else [sublist])]
    ...
    def get_annotation_file(self, name: str):
        ...
        df = self.load_csv(path, parse_dates=['start_datetime', 'end_datetime']).sort_values('start_datetime').reset_index(drop=True)
        ...
        setattr(self, f'{name}_annotator', annotators[0] if len(annotators) == 1 else annotators)
        setattr(self, f'{name}_annotation', annotations[0] if len(annotations) == 1 else annotations)
    ...
    def generate_dynamic_properties(self, attributes):
        ...
        for attr in attributes:
            if hasattr(self, f"_{attr}"):
                setattr(self.__class__, attr, property(self.getter_function(attr), self.setter_function(attr)))
