...
from utilities.def_func import suntime_hour
import csv
import pytz
import os
...
class LTAS:
    def __init__(self,
        ...
        if matrix is not None:
            assert np.issubdtype(matrix.dtype, np.number), 'Matrix must be an array of numbers'
            assert isinstance(freq, list) and all(isinstance(item, (int, float)) for item in freq), 'freq arg must be a list of numerical values'
            assert all(isinstance(item, pd.Timestamp) for item in time), 'time arg must be a list of datetimes'
            assert len(time) == matrix.shape[1], f'time vector is of length {len(time)} while matrix is of length {matrix.shape[1]}'
            assert len(freq) == matrix.shape[0], f'frequency vector is of length {len(time)} while matrix is of height {matrix.shape[0]}'

            matrix[:, -1] = 0  # set last welch to zero so that spectrograms appears blue in case of big gap in the data
            self.welch = matrix
            self.time = time
            self.freq = freq
            self.f_max = freq[-1]
            ...
            assert path.endswith('npz') or path.endswith('csv'), 'path arg must be a .npz or .csv'
    ...
    def plot_LTAS(self, output_path=None, output_name=None, dyn_min=40, dyn_max=80):
        ...
        if output_name is not None:
            plt.title(f"LTAS {output_name}\nfrom {self.time[0].strftime('%Y/%m/%d %H:%M')} to {self.time[-1].strftime('%Y/%m/%d %H:%M')}")
        else:
            plt.title(f"LTAS from {self.time[0].strftime('%Y/%m/%d %H:%M')} to {self.time[-1].strftime('%Y/%m/%d %H:%M')}")
    ...
    def plot_PSD(self, output_path, output_name):
        ...
        plt.title(f"PSD of signal from {self.time[0].strftime('%Y/%m/%d %H:%M')} to {self.time[-1].strftime('%Y/%m/%d %H:%M')}")
        
        if output_name is not None:
            plt.title(f"PSD {output_name}\nfrom {self.time[0].strftime('%Y/%m/%d %H:%M')} to {self.time[-1].strftime('%Y/%m/%d %H:%M')}")
        else:
            plt.title(f"PSD from {self.time[0].strftime('%Y/%m/%d %H:%M')} to {self.time[-1].strftime('%Y/%m/%d %H:%M')}")
        plt.tight_layout()
