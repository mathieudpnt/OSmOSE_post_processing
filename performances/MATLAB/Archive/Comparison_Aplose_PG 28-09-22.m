...
for i = 1:length(WavFolderInfo.wavList)
    nb_bin_int(i,1) = fix(WavFolderInfo.wavinfo(i).Duration/time_bin);
    last_bins(i,1) = mod(WavFolderInfo.wavinfo(i).Duration,time_bin);
...
time_vector_datetime =  [WavFolderInfo.wavDates_formated(1); WavFolderInfo.wavDates_formated(1) + cumsum(seconds(duration_det))];
time_vector = datenum(time_vector_datetime)*24*3600;
export_time2Raven(folder_data_wav, WavFolderInfo, time_vector, time_bin, duration_det) %Time vector as a Raven Table

elapsed_time.time_vector_creation = toc;

%% Creation of Aplose annotation vector
Ap_Annotation = importAploseSelectionTable(strcat(Ap_datapath,Ap_data_name),WavFolderInfo, last_bins, time_vector, index_exclude);
...
export_Aplose2Raven(WavFolderInfo, Ap_Annotation, Ap_datapath, Ap_data_name, folder_data_wav, time_vector)
...
% PG_Annotation.Begin_time = seconds(round( datetime(PG_Annotation.datetime_begin) - WavFolderInfo.wavDates_formated(1) ));
% PG_Annotation.End_time = seconds(round(datetime(PG_Annotation.datetime_end)-WavFolderInfo.wavDates_formated(1)));


% Creation of PG detection table in Raven output format
export_PG2Raven(PG_Annotation, folder_data_wav, WavFolderInfo)
...
export_Aplose2Raven(WavFolderInfo, PG_Annotation_formatted, Ap_datapath, Ap_data_name, folder_data_wav, time_vector, ' - PamGuard2Raven formatted Selection Table.txt')
...
file_name = [strcat(folder_data_wav,'\', WavFolderInfo.txt_filename, ' - results_',char(datetime(now,'ConvertFrom','datenum','Format','yyyy-MM-dd_HH-mm-ss')),'.txt')];
