function [data_table_Raven, data_table_Aplose] = importBinary(info_deploy, wav_struct, binary_struct, binary_folder, dt_deploy)
% this function extracts and converts detections from PAMGuard binary files

% sampling frequency
Fs = wav_struct.wavinfo(1).SampleRate;
...
% load data
data = loadPamguardBinaryFolder(binary_folder, '*.pgdf', 1);
if isempty(data)
...
Beg_sec = seconds(detection_begin - min(binary_struct.datetime)); % number of seconds between the first datetime and beginning of each detection
...
time_diff = diff(wav_struct.datetime);
