...
class TrajectoryFda:
    ...
    def setNewData(self, timestamp, latitude, longitude):
        ...
        if self.timestamp.size > 0:
            distanceNq = (
                self.coordinates2DistanceInMeters(
                    latitude, longitude, self.latitude[-1], self.longitude[-1]
                )
                / 1852.0
            )
    ...
    def getPosition(self, timestamp):
        currentLatFda = np.logical_and(
            self.latitudeResultArray[:, 0] <= timestamp,
            self.latitudeResultArray[:, 1] > timestamp,
        )
        currentLonFda = np.logical_and(
            self.longitudeResultArray[:, 0] <= timestamp,
            self.longitudeResultArray[:, 1] > timestamp,
        )
        ...
        lat_t = np.polyval(
            [
                self.latitudeResultArray[ii_lat, 2],
                self.latitudeResultArray[ii_lat, 3],
                self.latitudeResultArray[ii_lat, 4],
                self.latitudeResultArray[ii_lat, 5],
            ],
            timestamp,
        )
        lon_t = np.polyval(
            [
                self.longitudeResultArray[ii_lon, 2],
                self.longitudeResultArray[ii_lon, 3],
                self.longitudeResultArray[ii_lon, 4],
                self.longitudeResultArray[ii_lon, 5],
            ],
            timestamp,
        )
        return lat_t, lon_t
    ...
    def addNewLatitudeFunction(self, startTime, endTime, f, nbpointsCompressed):
        self.latitudeResultArray = np.vstack(
            (
                self.latitudeResultArray,
                [startTime, endTime, f[0], f[1], f[2], f[3], nbpointsCompressed],
            )
        )
        #        self.vidangeBuffer()
        return
    ...
    def addNewLongitudeFunction(self, startTime, endTime, f, nbpointsCompressed):
        self.longitudeResultArray = np.vstack(
            (
                self.longitudeResultArray,
                [startTime, endTime, f[0], f[1], f[2], f[3], nbpointsCompressed],
            )
        )
        return
    ...
    def processTrajectoryLongitude(self):
        ...
        if nbErr > 0:
            ...
            tdeb, tfin, f, nbErr2, iiFirstEr, nbPointsMaj = (
                self.processFunctionResearchMaj(
                    self.timestamp,
                    self.longitude,
                    self.epsilon,
    ...
    def processFunctionResearch(
        ...
        return (
            input_time[element2process[0]],
            input_time[element2process[-1]],
            result_f,
            nbPointsCompressed,
        )
    ...
    def vidangeBuffer(self):
        ...
        if sizeLat <= 1 or sizeLon <= 1:
            ...
            timestampLimite = (
                min(self.longitudeResultArray[-1, 0], self.latitudeResultArray[-1, 0])
                - 70
            )
    ...
    def processFunctionResearchMaj(
        ...
        return (
            ...
            ii,
            nbPointsCompressed,
        )  # TODO : il faut aussi retourner les heures de début et fin de tronçon

    def coordinates2DistanceInMeters(self, latA, lonA, latB, lonB):
        earthRadius = 6378137
        res = earthRadius * math.acos(
            math.sin(math.radians(latA)) * math.sin(math.radians(latB))
            + math.cos(math.radians(latA))
            * math.cos(math.radians(latB))
            * math.cos(math.radians(lonA - lonB))
        )
        return res
