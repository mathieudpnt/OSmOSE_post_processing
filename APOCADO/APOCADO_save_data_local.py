...
list_json = [
    ...
    for file_path in glob.glob(os.path.join(path, "**/metadata.json"), recursive=True)
]
...
for file_path in tqdm(list_json):
    ...
    ID = (
        "C"
        + str(metadata["campaign"])
        + "D"
        + str(metadata["deployment"])
        + " ST"
        + str(metadata["recorder"])
    )
    ...
    filename_LTAS = next(
        iter(
            glob.glob(
                os.path.join(os.path.dirname(file_path), "soundscape/LTAS**.npz"),
                recursive=True,
            )
        )
    )
    filename_PSD = next(
        iter(
            glob.glob(
                os.path.join(os.path.dirname(file_path), "soundscape/PSD**.npz"),
                recursive=True,
            )
        )
    )
    ...
    destination_path10 = os.path.join(
        destination_folder1, os.path.basename(filename_LTAS)
    )
    destination_path11 = os.path.join(
        destination_folder1, os.path.basename(filename_PSD)
    )
