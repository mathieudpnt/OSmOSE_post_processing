...
df = (
    pd.read_csv(file, sep=",", parse_dates=["start_datetime", "end_datetime"])
...
df["start_datetime"] = [
    dt.tz_convert(pytz.FixedOffset(60)) for dt in df["start_datetime"]
]
df["end_datetime"] = [
    dt.tz_convert(pytz.FixedOffset(60)) for dt in df["start_datetime"]
]
...
df["start_datetime"] = [
    x.strftime("%Y-%m-%dT%H:%M:%S.000") + "+01:00" for x in df["start_datetime"]
]
df["end_datetime"] = [
    x.strftime("%Y-%m-%dT%H:%M:%S.000") + "+01:00" for x in df["end_datetime"]
]

file_out = os.path.join(
    os.path.dirname(file), os.path.basename(file).split(".csv")[0] + "_WD_added.csv"
)
