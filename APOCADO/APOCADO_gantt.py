...
data = pd.read_excel(
    "L:/acoustock/Bioacoustique/DATASETS/APOCADO/PECHEURS_2022_PECHDAUPHIR_APOCADO/APOCADO - Suivi déploiements.xlsm",
    skiprows=[0],
)
data = data[data["check heure Raven"] == 1].reset_index(drop=True)

data["datetime deployment"] = [
    pd.Timestamp.combine(data["date deployment"][i].date(), data["time deployment"][i])
    for i in range(len(data))
]
data["datetime recovery"] = [
    pd.Timestamp.combine(data["date recovery"][i].date(), data["time recovery"][i])
    for i in range(len(data))
]

data_gant = pd.DataFrame(
...
ax.set_xlim(
    data_gant["dt_deployment"].min() - dt.timedelta(days=7),
    data_gant["dt_recovery"].max() + dt.timedelta(days=7),
)
...
season_dates = pd.date_range(
    start=data["date deployment"].min(), end=data["date recovery"].max(), freq="D"
)
...
legend_elements = [
    ...
    for dep in sorted(list(data_gant[arg].unique()))
]
legend = ax1.legend(
