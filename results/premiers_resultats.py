...
root = Tk()
root.withdraw()
detections_file = filedialog.askopenfilename(title="Select APLOSE format detection file", filetypes=[("CSV files", "*.csv")])
...
t_detections = sorting_annot_boxes(detections_file, tz_data, date_begin, date_end)
...
plt.xlim(t_rounder(date_begin), t_rounder(date_end))
