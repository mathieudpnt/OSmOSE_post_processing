...
df_detections, t_detections = sorting_detections(
    files_list, tz=pytz.UTC, timebin_new=60
)
...
[_, _, dt_dusk, dt_dawn, dt_day, dt_night] = suntime_hour(
    begin_deploy, end_deploy, tz_data, lat, lon
)
...
for idx_day, day in enumerate(list_days):
    ...
    a = (nb_det_day[idx_day] + nb_det_night[idx_day]) / (
        night_duration_dec[idx_day] + day_duration_dec[idx_day]
    )
...
print(
    "DAY\n***\nShapiro-Wilk {0} \naverage {1} \nvariance {2} \n***".format(
        s_day, av_day, var_day
    )
)
print(
    "NIGHT\n***\nShapiro-Wilk {0} \naverage {1} \nvariance {2}\n***".format(
        s_night, av_night, var_night
    )
)
