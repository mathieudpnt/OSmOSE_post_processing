...
from utilities.SoundTrap import *
import matplotlib.image as mpimg

# %% Load data
with open(r'L:\acoustock\Bioacoustique\DATASETS\APOCADO\PECHEURS_2022_PECHDAUPHIR_APOCADO\data_Deployment.pkl', 'rb') as f:
    data = pickle.load(f)
...
list_signal = [file_path for path in path_acoustock for file_path in glob.glob(os.path.join(path, r"**\signal.ravensignal"), recursive=True)]
list_ltas_csv = [file_path for path in path_acoustock for file_path in glob.glob(os.path.join(path, r"**\ltsa.csv"), recursive=True)]
list_ltas = [file_path for path in path_acoustock for file_path in glob.glob(os.path.join(path, r"**\ltsa.ravenltsa"), recursive=True)]
list_img = [file_path for path in path_acoustock for file_path in glob.glob(os.path.join(path, r"**\soundscape\LTAS**.png"), recursive=True)]
list_img2 = [file_path for path in path_acoustock for file_path in glob.glob(os.path.join(path, r"**\soundscape\PSD**.png"), recursive=True)]
list_npz = [file_path for path in path_acoustock for file_path in glob.glob(os.path.join(path, r"**\soundscape\LTAS**.npz"), recursive=True)]
# %%
...
for i in tqdm(range(len(list_signal))):
    ...
    path_splitted = json.loads(text.decode('latin-1'))['fileList'][0]['filePath']['path'].split('\\')
...
data_LTAS = pd.DataFrame(list(zip(campaign_LTAS, recorder_LTAS, begin_LTAS, time_resolution_LTAS, frequency_resolution_LTAS, list_ltas_csv)), columns=['campaign_LTAS', 'recorder_LTAS', 'begin_LTAS', 'time_resolution_LTAS', 'frequency_resolution_LTAS', 'csv']).sort_values(['campaign_LTAS', 'begin_LTAS'])

# %% Select deployment

name, campaign, deployement, recorder, gain, begin_date, deployment, recovery, csv_LTAS, check_LTAS, time_resolution, frequency_resolution, output_folder = [], [], [], [], [], [], [], [], [], [], [], [], []
for i, d in enumerate(data):
    ...
    LTAS_file = data_LTAS[(data_LTAS['campaign_LTAS'] == d.campaign) & (data_LTAS['recorder_LTAS'] == d.recorder)].iloc[-1]

    dt_LTAS = LTAS_file['begin_LTAS']
    begin_date_naive = extract_datetime(os.path.basename(glob.glob(os.path.join(d.wav_folder, "*.wav"))[0]))
...
df = pd.DataFrame(list(zip(name, campaign, deployement, recorder, gain, begin_date, deployment, recovery, csv_LTAS, time_resolution, frequency_resolution, output_folder)), columns=['name', 'campaign', 'deployement', 'recorder', 'gain', 'begin_campaign', 'deployment', 'recovery', 'csv_LTAS', 'time_resolution_LTAS', 'frequency_resolution_LTAS', 'output_folder']).sort_values(['campaign', 'deployment'])


# %% plot LTAS and save figure

for i in tqdm(range(len(df))):
    LTAS_deploy = LTAS(path=df['csv_LTAS'][i],
    ...
    LTAS_deploy.plot_LTAS(output_path=out_folder,
                          output_name=df['name'][i],
                          dyn_min=20,
                          dyn_max=100)
...
for i in filter_img:
    ...
    LTAS_plot.append(LTAS_load.plot_LTAS(output_path=None,
                                         output_name=None,
                                         dyn_min=20,
                                         dyn_max=100))
    
    plt.imshow(LTAS_load.welch[:-1, :-1],
               aspect='auto',
               vmin=20,
               vmax=100,
               origin='lower',
               extent=[LTAS_load.time[0], LTAS_load.time[-1], LTAS_load.freq[0], LTAS_load.freq[-1]],
               )

fig, ax = plt.subplots()
plt.plot(LTAS_load.freq[:-1],
         PSD[0],
         label="C4D7 ST336363566")
plt.plot(LTAS_load.freq[:-1],
         PSD[1],
         label="C1D2 ST336363566")
