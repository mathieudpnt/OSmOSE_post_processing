...
df_detections, t_detections = sorting_detections(
    files_list, timebin_new=60, tz=pytz.FixedOffset(120)
)
...
if dt_mode == "fixed":
    ...
    begin_date = pd.to_datetime(
        timestamps_file["timestamp"].iloc[0], format="%Y-%m-%dT%H:%M:%S.%f%z"
    )
    end_date = pd.to_datetime(
        timestamps_file["timestamp"].iloc[-1], format="%Y-%m-%dT%H:%M:%S.%f%z"
    ) + dt.timedelta(seconds=time_bin[0])
elif dt_mode == "input":
...
def get_season(
    date,
):  # winter : december -> february, spring : march -> may, summer : june to august, autumn : september to november
...
# Ask which site
site = easygui.buttonbox(
    msg="On which CETIROISE site are you working ?",
    choices=("A", "B", "C", "D", "G", "E", "F"),
)
