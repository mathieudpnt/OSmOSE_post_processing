...
list_json = [file_path for path in path_acoustock for file_path in glob.glob(os.path.join(path, r"**\**metadata.json"), recursive=True)]
...
test2 = Deployment(campaign='Point A',
                   deployment='phase B',
                   recorder='hihu',
...
# %%
plot_single(data=deploy[0], file='pamguard',
            timebin=10,
            )
# %%
get_perf(data=deploy,
