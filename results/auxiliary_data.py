...
def df_1annot_1label(df0, annotator, label):
    ...
    df3["start_datetime"] = pd.to_datetime(
        df3.start_datetime, format="%d/%m/%Y %H:%M:%S%tz"
    )
    df3["end_datetime"] = pd.to_datetime(
        df3.end_datetime, format="%d/%m/%Y %H:%M:%S%tz"
    )
...
def CreatVec_datetime_det(df_results, annotator, label_test):
    ...
    det_annot_ref_label = det_annot_ref.loc[
        det_annot_ref["annotation"].isin([label_test])
    ]
    ...
    beg_det_ref = [
        calendar.timegm(L.timetuple()) for L in det_annot_ref_label["start_datetime"]
    ]
    end_det_ref = [
        calendar.timegm(L.timetuple()) for L in det_annot_ref_label["end_datetime"]
    ]
...
def sorting_annot_boxes(FileDet, tz, date_begin, date_end):
    ...
    df4["start_datetime"] = pd.to_datetime(
        df4["start_datetime"], format="%Y-%m-%dT%H:%M:%S.%f%Z"
    )
    df4["end_datetime"] = pd.to_datetime(
        df4["end_datetime"], format="%Y-%m-%dT%H:%M:%S.%f%Z"
    )

    df4["start_datetime"] = [
        y.tz_convert(tz) for x, y in enumerate(df4["start_datetime"])
    ]  # converting to desired tz
    df4["end_datetime"] = [
        y.tz_convert(tz) for x, y in enumerate(df4["end_datetime"])
    ]  # converting to desired tz

    df5 = df4[
        (df4["start_datetime"] >= date_begin) & (df4["start_datetime"] <= date_end)
    ]  # select data within [date_begin;date_end]
...
def get_duration(timestamp_file, wav_path, TimeZ):
    ts = pd.read_csv(
        timestamp_file, delimiter=",", header=None, names=["name", "timestamp"]
    )
    ts["timestamp"] = pd.to_datetime(
        ts["timestamp"], format="%Y-%m-%dT%H:%M:%S.%fZ"
    )  # from str to naïve datetime
    ts["timestamp"] = [
        pytz.timezone(TimeZ).localize(y) for x, y in enumerate(ts["timestamp"])
    ]  # add timezone
...
def plot_tides(data_path, date_begin, date_end, tz):
    ...
    df2_maree = df_maree[
        (df_maree["# Date"] <= date_end) & (df_maree["# Date"] >= date_begin)
    ]  # sorting
...
def res_timebin_plot(date_begin, date_end, duration_min):
    ...
    if res_min.isnumeric() == False:
        ...
        if duration_min % res_min == 0:
            date_list = [
                date_begin + dt.timedelta(minutes=res_min * x)
                for x in range(duration_min // res_min)
            ]  # 24*60 = 144*10 min in 24h
...
# %% User input
date_begin = pytz.timezone(tz_data).localize(
    pd.to_datetime(
        easygui.enterbox("datetime begin ? (yyyy MM dd HH mm ss) :"),
        format="%Y %m %d %H %M %S",
    )
)
date_end = pytz.timezone(tz_data).localize(
    pd.to_datetime(
        easygui.enterbox("datetime end ? (yyyy MM dd HH mm ss) :"),
        format="%Y %m %d %H %M %S",
    )
)
...
if sum(test_ts) == 0:
    print("Aucun fichier compris entre", str(date_begin), "et", str(date_end))
else:
    print(
        sum(test_ts),
        "/",
        len(ts),
        "fichiers compris entre",
        str(date_begin),
        "et",
        str(date_end),
    )
...
if duration_h.is_integer() == True:
    ...
    print("duration_h is not an integer")

if duration_min.is_integer() == True:
    duration_min = int(duration_min)
else:
    print("duration_min is not an integer")
...
counter_label = (
    df1.groupby("annotation")["annotator"].apply(Counter).unstack(fill_value=0)
)
print(counter_label)
...
plt.title(
    "Number of detections of each annotator, classified by label type", fontsize=40
)
...
for num_annot, x in enumerate(annotators):
    ...
    Annot_detect = np.zeros(
        (1, nDetect)
    )  # Array of 0 and 1 with length = number of detections of annotator 'x' -> 1 if the annotation matches with one annot_ref annotation
    Ref_detect = np.zeros(
        (1, L)
    )  # Array of 0 and 1 with length = number of detections of annotator annot_ref -> 1 if the annotation matches with one annotatator 'x' annotation
    ...
    print(
        "Nombre de détections manquées par l'autre annotateur :", Missed_det[num_annot]
    )
    print(
        "Nombre de détections que l'autre annotateur a, mais pas vous :",
        False_alarm[num_annot],
    )
...
data_hist = ax.hist(df_1a1l["start_datetime"], bins=date_list, color="coral")
# histo annotation
...
ax.set_ylabel(
    "taux d'annotation / " + str(res_min) + " min", fontsize=20, color="coral"
)
...
fig.suptitle(
    annot_ref + " " + label_ref + date_begin.strftime(" - %d/%m/%y UTC%z"),
    fontsize=24,
    y=0.95,
)
...
fig.suptitle(
    "Annotations de " + annot_ref + " du" + date_begin.strftime(" %d/%m/%y UTC%z"),
    fontsize=24,
    y=0.95,
)
...
for i, L in enumerate(selected_labels):
    ...
    ax[i].xaxis.set_major_formatter(
        mdates.DateFormatter("%H:%M", tz=pytz.timezone(tz_data))
    )
    ...
    ax[i].set_ylabel(
        "taux d'annotation / " + str(res_min) + " min", fontsize=20, color="tab:red"
    )
...
def read_pkl(pkl_filename):
    ...
    time_welch.sort()
    datetime_welch = [
        (dt.datetime.strptime(x, "%Y-%m-%d %H:%M:%S.%f")) for x in time_welch
    ]
...
def plot_noise(tz_data, date_begin, date_end, pkl_filename):
    ...
    time_welch = [
        pytz.timezone(tz_data).localize(y) for x, y in enumerate(time_welch)
    ]  # add timezone
...
ax.hist(df_1a1l["start_datetime"], bins=date_list, color="coral")
# histo annotation
...
def CreatVec_datetime_det(df_results, annotator, label):
    ...
    # Read the start and end timestamp of each detection of 'annot_ref'
    beg_det_struc_time = [
        (dt.datetime.strptime(x, "%Y-%m-%dT%H:%M:%S.%f+00:00").timetuple())
        for x in det_annot_ref_label["start_datetime"]
    ]
    beg_det_datetime = [
        (dt.datetime.strptime(x, "%Y-%m-%dT%H:%M:%S.%f+00:00"))
        for x in det_annot_ref_label["start_datetime"]
    ]
...
[hour_sunrise, hour_sunset] = suntime_hour(
    date_begin_rounded_str, date_end_rounded_str, "Europe/Paris", lat, lon, 2
)
