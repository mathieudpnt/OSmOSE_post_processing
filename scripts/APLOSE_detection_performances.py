...
annotators_detections = (
    info["annotators"].explode().unique()[0]
    if len(info["annotators"].explode().unique()) == 1
    else list(set(info["annotators"].explode()))
)
labels_detections = (
    info["labels"].explode().unique()[0]
    if len(info["labels"].explode().unique()) == 1
    else list(set(info["labels"].explode()))
)
if len(info["max_time"].explode().unique()) == 1:
    timebin_detections = int(info["max_time"].unique()[0])
else:
    raise Exception("More than one timebin passed")
if len(info["timestamp_file"].explode().unique()) == 1:
    timestamp_file = info["timestamp_file"].explode().unique()[0]
else:
    raise Exception("More than one timestamp file passed")
...
if len(parameters) == 2:
    choice_ref = easygui.buttonbox(
        "Select the reference",
        "{0}".format("Loading data"),
        [os.path.basename(parameters[i]["file"]) for i in range(len(parameters))],
    )
    if os.path.basename(parameters[0]["file"]) != choice_ref:
        info.iloc[0], info.iloc[1] = info.iloc[1], info.iloc[0]
else:
    raise Exception("Passed sets of parameters different than 2")
...
if mode == "timestamps":
    begin_date = pd.read_csv(timestamp_file, parse_dates=["timestamp"])[
        "timestamp"
    ].iloc[0]
    end_date = pd.read_csv(timestamp_file, parse_dates=["timestamp"])["timestamp"].iloc[
        -1
    ] + pd.Timedelta(timebin_detections, unit="second")
elif mode == "custom":
...
t2 = t_rounder(
    min(timestamp_range[-1], end_date), res=timebin_detections
) + pd.Timedelta(seconds=timebin_detections)
time_vector = [
    ts.timestamp()
    for ts in pd.date_range(start=t, end=t2, freq=str(timebin_detections) + "s")
]
...
selected_annotator1 = (
    easygui.buttonbox(
        "Select annotator 1 (reference)",
        "file 1 : {0}".format(files_list[0].split("/")[-1]),
        annotators1,
    )
    if len(annotators1) > 1
    else annotators1[0]
)
selected_label1 = (
    easygui.buttonbox(
        "Select a label", "file 1 : {0}".format(files_list[0].split("/")[-1]), labels1
    )
    if len(labels1) > 1
    else labels1[0]
)
selected_annotations1 = df_detections[
    (df_detections["annotator"] == selected_annotator1)
    & (df_detections["annotation"] == selected_label1)
    & (df_detections["start_datetime"] >= begin_date)
    & (df_detections["end_datetime"] <= end_date)
]
...
for i in range(len(times1_beg)):
    for j in range(k, len(time_vector) - 1):
        if int(times1_beg[i] * 1e7) in range(
            int(time_vector[j] * 1e7), int(time_vector[j + 1] * 1e7)
        ) or int(times1_end[i] * 1e7) in range(
            int(time_vector[j] * 1e7), int(time_vector[j + 1] * 1e7)
        ):
            ranks.append(j)
            k = j
...
selected_annotator2 = (
    ...
    if len(annotators2) > 1
    else annotators2[0]
)
selected_label2 = (
    easygui.buttonbox(
        "Select a label", "{0}".format(files_list[1].split("/")[-1]), labels2
    )
    if len(labels2) > 1
    else labels2[0]
)
selected_annotations2 = df_detections[
    (df_detections["annotator"] == selected_annotator2)
    & (df_detections["annotation"] == selected_label2)
    & (df_detections["start_datetime"] >= begin_date)
    & (df_detections["end_datetime"] <= end_date)
]
...
for i in range(len(times2_beg)):
    for j in range(k, len(time_vector) - 1):
        if int(times2_beg[i] * 1e7) in range(
            int(time_vector[j] * 1e7), int(time_vector[j + 1] * 1e7)
        ) or int(times2_end[i] * 1e7) in range(
            int(time_vector[j] * 1e7), int(time_vector[j + 1] * 1e7)
        ):
            ranks.append(j)
            k = j
...
delta, start_vec, end_vec = (
    ...
    t_rounder(end_date + pd.Timedelta(seconds=timebin_detections), res=600),
)
bin_vector = [
...
n_annot_max = (
    res_min * 60
) / timebin_detections  # max nb of annoted time_bin max per res_min slice

fig, ax = plt.subplots(1, 1, dpi=200, figsize=(10, 4))

hist_plot = ax.hist(
    [selected_annotations1["start_datetime"], selected_annotations2["start_datetime"]],
    bins=bin_vector,
...
fig.suptitle(
    "[{0}/{1}] VS [{2}/{3}]".format(
        selected_annotator1, selected_label1, selected_annotator2, selected_label2
    ),
    y=1.02,
)
