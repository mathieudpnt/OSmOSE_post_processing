...
df_pamguard, t_pamguard = sorting_detections(
    files=pamguard_path, timebin_new=60, tz=pytz.FixedOffset(120)
)
...
if dt_mode == "auto":
    ...
    end_deploy = extract_datetime(
        filemetadata["filename"].iloc[-1], tz_data
    ) + dt.timedelta(seconds=filemetadata["duration"].iloc[-1])
    ...
    end_deploy = extract_datetime(
        timestamps_file["filename"].iloc[-1], tz_data
    ) + dt.timedelta(seconds=durations[-1])
...
time_vector = [
    ...
    for elem in wav_datetimes[i].timestamp()
    + np.arange(0, durations[i], time_bin).astype(int)
]
time_vector_str = [
    str(wav_names[i]).split(".wav")[0] + "_+" + str(elem)
    for i in range(len(wav_names))
    for elem in np.arange(0, durations[i], time_bin).astype(int)
]


## Pamguard
times_PG_beg = [
    df_pamguard["start_datetime"][i].timestamp() for i in range(len(df_pamguard))
]
times_PG_end = [
    df_pamguard["end_datetime"][i].timestamp() for i in range(len(df_pamguard))
]

PG_vec, ranks, k = np.zeros(len(time_vector), dtype=int), [], 0
for i in range(len(times_PG_beg)):
    for j in range(k, len(time_vector) - 1):
        if int(times_PG_beg[i] * 1000) in range(
            int(time_vector[j] * 1000), int(time_vector[j + 1] * 1000)
        ) or int(times_PG_end[i] * 1000) in range(
            int(time_vector[j] * 1000), int(time_vector[j + 1] * 1000)
        ):
            ranks.append(j)
            k = j
...
while True:
    ...
    selected_time_vector, selected_time_vector_str, selected_PG_vec, selected_dates = (
        n_random_hour(
            time_vector_ts=time_vector,
            time_vector_str=time_vector_str,
            vec=PG_vec,
            n_hour=3,
            tz=tz_data,
            time_step=time_bin,
        )
    )  # select randomly n hour in the dataset
    if round(sum(selected_PG_vec) / len(selected_time_vector), 3) > 0.75 * round(
        sum(PG_vec) / len(time_vector), 3
    ) and round(sum(selected_PG_vec) / len(selected_time_vector), 3) < 1.25 * round(
        sum(PG_vec) / len(time_vector), 3
    ):
        break
...
print(
    "\t original :", round(sum(PG_vec) / len(time_vector), 3)
)  # proportion de positifs dans les éléments
print(
    "\tselection :", round(sum(selected_PG_vec) / len(selected_time_vector), 3)
)  # proportion de positifs dans les éléments selectionnés randomly


# %% EXPORT TO RAVEN FORMAT

double_check_dir = os.path.join(os.path.dirname(pamguard_path), "Double_check")
if not os.path.exists(double_check_dir):
    os.mkdir(double_check_dir)
double_check_path = double_check_dir

result_dir = os.path.join(
    double_check_path, dt.datetime.now().strftime("%Y-%m-%dT%H_%M_%S")
)
...
df0_PG2Raven = export2Raven(
    wav_tuple, time_vector, time_vector_str, 0.9 * fmax, selection_vec=None
)
PG2Raven_str0 = (
    "PG_double_check0_all_"
    + t_rounder(wav_datetimes[0], res=3600).strftime("%y%m%d")
    + "_"
    + t_rounder(wav_datetimes[-1], res=3600).strftime("%y%m%d")
    + "_"
    + str(time_bin)
    + "s"
    + ".txt"
...
df1_PG2Raven = export2Raven(
    wav_tuple, time_vector, time_vector_str, 0.8 * fmax, selection_vec=PG_vec
)
PG2Raven_str1 = (
    "PG_double_check1_all_positives_"
    + t_rounder(wav_datetimes[0]).strftime("%y%m%d")
    + "_"
    + t_rounder(wav_datetimes[-1]).strftime("%y%m%d")
    + "_"
    + str(time_bin)
    + "s"
    + ".txt"
...
df3_PG2Raven = export2Raven(
    wav_tuple,
    selected_time_vector,
    selected_time_vector_str,
    0.4 * fmax,
    selection_vec=selected_PG_vec,
)
PG2Raven_str3 = (
    "PG_double_check3_selected_positives_"
    + t_rounder(wav_datetimes[0]).strftime("%y%m%d")
    + "_"
    + t_rounder(wav_datetimes[-1]).strftime("%y%m%d")
    + "_"
